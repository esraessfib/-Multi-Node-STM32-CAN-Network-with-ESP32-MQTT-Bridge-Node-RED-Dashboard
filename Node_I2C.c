...
void CAN_Send_Temperature(float temp)
{
    ...
    TxMessage.Data[0] = (0x681  & 0xFF); 
    TxMessage.Data[1] = (temp_fixed >> 8)& 0xFF;   // MSB
	  TxMessage.Data[2] = (temp_fixed & 0xFF); // LSB
...
/*
   Gestionnaire d'interruption CAN RX FIFO0
   ISR courte - juste toggle LED, le traitement se fait dans main()
*/
/*void CAN1_RX0_IRQHandler(void)
{
    / Vérifier si message présent dans FIFO0 /
    if(CAN_GetITStatus(CAN1, CAN_IT_FMP0) != RESET)
    {
        / Toggle LED orange = message reçu /
        //GPIOD->ODR ^= (1 << 13);
        bol=1;
        / Note: On ne lit PAS le message ici, on le laisse dans le FIFO /
        / Le main() va détecter via RF0R.FMP0 qu'il y a un message /
        / et le traitera dans la boucle principale /
    }
}*/
...
int main(void)
{
    ...
    while(1) 
    {
      /*  // Vérifier si un message CAN est en attente dans FIFO0 
        // Lecture directe du registre matériel RF0R.FMP0 (bits 1-0) 
        if((CAN1->RF0R & CAN_RF0R_FMP0) != 0)
        {
            CanRxMsg RxMessage;
            
            // Message présent dans FIFO0 - le lire 
            CAN_Receive(CAN1, CAN_FIFO0, &RxMessage);
            
            // Vérifier si c'est une requête de température 
            if(RxMessage.StdId == CAN_ID_TEMP_REQUEST ) 

            {
                // Debug UART 
                USART3_send_string("CAN RX: Request received\r\n");
                
                // Lire température et envoyer réponse 
                current_temperature = I2C_ReadTemperature();
                CAN_Send_Temperature(current_temperature);
							  
							  // Affichage via UART pour debug 
                sprintf(temp_str, "Temperature: %.1f C\r\n", current_temperature);
                USART3_send_string(temp_str);
            }
            
  // Le message est automatiquement retiré du FIFO par CAN_Receive() 
        }*/
